...
def handle_join(data):
    ...
    # Always join the vehicle room for broadcasting
    join_room(f'chat_{vehicle_token}')
    
    # If it's a specific session, join that room too
    if visitor_session_id:
...
def handle_send_message(data):
    ...
    if visitor_session_id:
        if isinstance(visitor_session_id, str) and visitor_session_id.isdigit():
            visitor_session_id = int(visitor_session_id)
        elif isinstance(visitor_session_id, int):
            visitor_session_id = visitor_session_id
        else:
            visitor_session_id = None
    else:
    ...
    # Emit to the specific session room and general vehicle room to ensure delivery
    socketio.emit('receive_message', payload, room=payload['room'])
    if visitor_session_id:
        socketio.emit('receive_message', payload, room=f'chat_{vehicle_token}')
